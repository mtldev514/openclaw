...
|---|---|---|
| dev | `npm run dev` | node scripts/run-node.mjs |
| start | `npm start` | node scripts/run-node.mjs |
| build | `npm run build` | pnpm canvas:a2ui:bundle && tsdown && node --import tsx scripts/canvas-a2ui-copy.ts && node --import tsx scripts/copy-hook-metadata.ts && node --import tsx scripts/write-build-info.ts && node --import tsx scripts/write-cli-compat.ts |
| test | `npm run test` | node scripts/test-parallel.mjs |
| lint | `npm run lint` | oxlint --type-aware |
| format | `npm run format` | oxfmt --check |
| android:assemble | `npm run android:assemble` | cd apps/android && ./gradlew :app:assembleDebug |
| android:install | `npm run android:install` | cd apps/android && ./gradlew :app:installDebug |
| android:run | `npm run android:run` | cd apps/android && ./gradlew :app:installDebug && adb shell am start -n ai.openclaw.android/.MainActivity |
| android:test | `npm run android:test` | cd apps/android && ./gradlew :app:testDebugUnitTest |
| canvas:a2ui:bundle | `npm run canvas:a2ui:bundle` | bash scripts/bundle-a2ui.sh |
| check | `npm run check` | pnpm tsgo && pnpm lint && pnpm format |
| check:loc | `npm run check:loc` | node --import tsx scripts/check-ts-max-loc.ts --max 500 |
| docs:bin | `npm run docs:bin` | node scripts/build-docs-list.mjs |
| docs:build | `npm run docs:build` | cd docs && pnpm dlx --reporter append-only mint broken-links |
